...
随着 Hermes Agent 的持续演进，内置技能库不断扩展，覆盖更多专业场景。以下为近期新增的重要技能：

osint-investigation
=====================

OSINT（Open Source Intelligence，开源情报）调查技能。该技能为 Agent 提供系统化的开源信息收集和分析能力，包括：

- 多源信息聚合（公开数据库、社交媒体、域名注册信息等）
- 人员/实体关联分析
- 信息交叉验证与可信度评估
- 调查报告自动生成

适用于安全审计、背景调查、竞争情报等专业场景。

darwinian-evolver
==================

自主进化式代码改进技能。该技能实现了"达尔文式"的代码优化循环：

1. **变异** ：对目标代码生成多个变体（不同实现策略）。
2. **评估** ：通过测试套件和代码质量指标对每个变体评分。
3. **选择** ：保留最优变体，淘汰劣质变体。
4. **迭代** ：重复上述过程，逐步提升代码质量。

该技能适合在 CI/CD 管道中自动运行，持续优化关键模块的性能和可维护性。

pinggy-tunnel
==============

SSH 隧道技能，通过 `Pinggy <https://pinggy.io>`_ 服务快速建立公网可访问的隧道。

核心特性：

- 一条命令即可将本地端口暴露到公网，无需安装客户端
- 支持 HTTP/HTTPS/TCP 隧道
- 自动生成公网 URL，方便分享和测试
- 适合临时调试 Webhook、预览本地开发服务器等场景

EVM 多链技能
=============

EVM（Ethereum Virtual Machine）多链交互技能，支持 8 条区块链网络和 14 个常用命令。

支持的网络（8 条链）：

- Ethereum 主网、Polygon、Arbitrum、Optimism、Base、BSC、Avalanche C-Chain、Fantom

支持的命令（14 个）：

- 余额查询、代币转账、合约读取/写入、事件日志查询、Gas 估算、交易状态追踪等

该技能封装了 Web3.py 的常用操作，Agent 无需了解底层 RPC 细节即可完成链上交互。

HuggingFace 技能 Tap
======================

HuggingFace 技能库（``huggingface/skills``）已被添加为 Hermes 的可信默认 Tap（技能源）。这意味着用户无需手动配置，即可直接搜索和安装 HuggingFace 生态中的技能。

可信 Tap 的含义：

- **安全扫描豁免** ：来自可信 Tap 的技能仍会经过安全扫描，但不需要用户手动确认安装来源。
- **自动发现** ：``hermes skills search`` 命令会同时搜索本地和 HuggingFace Tap。
- **版本追踪** ：安装的技能会记录来源 Tap 和版本号，支持后续更新检查。

Notion 技能重构
================

Notion 技能已全面重构，适配 2026 年 5 月发布的 Notion Developer Platform 新版 API。

主要变更：

- **新版认证** ：迁移至 Notion Developer Platform 的 OAuth 2.0 流程，取代原有的 Integration Token 直接认证。
- **数据库操作增强** ：支持新版 API 的数据库查询过滤和排序语法，查询性能提升显著。
- **页面内容丰富类型支持** ：新增对 Notion 公式块（``equation``）、嵌入块（``embed``）、书签块（``bookmark``）等高级内容类型的读写支持。
- **批量操作优化** ：利用新版 API 的批量端点，减少 API 调用次数，大幅提升大量页面创建和更新的效率。
