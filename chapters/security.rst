...
``_CONTEXT_THREAT_PATTERNS`` 定义了 10 种正则表达式模式，覆盖主要的提示注入攻击类型：
...
``tools/approval.py`` 中的 ``DANGEROUS_PATTERNS`` 定义了 38+ 个正则表达式模式，
覆盖以下危险操作类别。模式列表参照 Claude Code 的危险命令检测进行了加强，
新增了对 ``sudo`` 提权变体的捕获：
...
- 捕获 ``sudo`` 与 ``-S``（stdin 密码）、``-A``（askpass）、
  ``-i``/``-s``（shell 模式）等特权标志的组合
- 当 ``SUDO_PASSWORD`` 环境变量未设置时，阻止 ``sudo -S`` 密码猜测行为
- 防止通过 ``stdin``、``askpass`` 或 ``shell`` 标志绕过交互式密码提示
...
OAuth PKCE state/verifier 分离
=================================

在 OAuth 2.1 PKCE 认证流程中，``state`` 参数和 ``code_verifier`` 现在使用
**独立的随机值** 生成。之前的实现中，同一个值被用于两者，这构成了一个安全漏洞——
如果 ``state`` 泄露（例如通过 Referer 头），攻击者可以推导出 ``code_verifier``，
从而伪造授权码交换请求。分离后，即使 ``state`` 被截获，也不会危及 PKCE 的
安全性。

Shell Hook 阻断统一
=====================

``_block_message`` 辅助函数从 ``_parse_response`` 中被提取出来，
统一了工具调用的阻断逻辑。此前，当插件通过 ``pre_tool_call`` 钩子
阻断工具调用时，如果阻断消息（``message``）或原因（``reason``）缺失，
阻断行为会被跳过。修复后，即使消息字段为空，阻断仍然生效——
系统使用默认的 "Blocked by plugin policy" 消息。

工具错误字符串净化
===================

工具调用产生的错误字符串在注入到模型上下文之前，现在会经过 ``_sanitize_error()``
净化处理（#26823）。这防止了工具错误消息中可能包含的提示注入内容——
例如，一个恶意 MCP 工具故意返回包含指令的错误消息——被模型误读为
合法指令并执行。
...
1.  **提示注入防御** ：10 种正则模式 + 不可见 Unicode 检测，阻止上下文文件注入
2.  **MCP 安全** ：工具描述扫描、环境变量过滤、凭证错误净化、工具名称清洗
3.  **审批系统** ：38+ 种危险命令模式检测（含 sudo 提权加固），三级审批模式，会话级状态管理
4.  **凭证管理** ：.env 隔离、来源抑制、显式配置门控、跨进程文件锁、OAuth PKCE state/verifier 分离
5.  **沙箱执行** ：Docker/SSH/Modal 容器隔离，环境变量清理，资源限制
6.  **工具冲突保护** ：命名空间前缀、注册顺序保护、Shell Hook 阻断统一
7.  **预执行扫描** ：Tirith 安全扫描器集成、工具错误字符串净化
